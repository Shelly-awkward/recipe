...
window.RECIPES = [
  ...
  },
  {
    ...
    steps: [
      ...
      "放進 BS607，選純微波、最低火力（約 150 到 200W），先加熱 4 分鐘。",
      "打開看：邊緣凝固、中間還會晃，就是對的。再用同火力每次 2 分鐘往上加。",
      "加到輕輕搖杯子時，中心整片一起晃、表面不流動就好了。兩杯大約共 8 到 12 分鐘。",
      "取出靜置 3 分鐘，用餘溫把中心悶熟。撒上蔥花。"
    ],
    notes: "蛋和液體的比例是 1 比 3，這個比例不要改，改了口感就不對。BS607 的微波火力有分段，選最低那一段；如果最低只到 300W，時間就抓上面的七成，並且每 1 分鐘確認一次。想更保險可以改用蒸氣模式：一樣蓋保鮮膜留縫，100°C 蒸 12 到 15 分鐘，比較慢但比較不會失敗。只做一杯時間大約減半，還是要分段確認。微波時間受杯子材質和蛋液高度影響很大，第一次做寧可短一點多看幾次，過頭就回不去了。"
  }
];
